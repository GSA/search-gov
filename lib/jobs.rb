...
module Jobs
  ...
  def self.establish_connection!
    @endpoint  = ENV['USAJOBS_ENDPOINT']          || Rails.application.secrets.jobs[:endpoint]
    host       = ENV['USAJOBS_HOST']              || Rails.application.secrets.jobs[:host]
    key        = ENV['USAJOBS_AUTHORIZATION_KEY'] || Rails.application.secrets.jobs[:authorization_key]
    user_agent = ENV['USAJOBS_USER_AGENT']        || Rails.application.secrets.jobs[:user_agent]
    adapter    = ENV['USAJOBS_ADAPTER']           || Rails.application.secrets.jobs[:adapter]
