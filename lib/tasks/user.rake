namespace :usasearch do
  namespace :user do
    ...
    task update_approval_status: :environment do
      set_user_not_approved(User.approved_affiliate,
    ...
    task update_inactive_approval_status: :environment do
      set_user_not_approved(User.not_active, 'has been inactive for 90 days')
    end
  end

  def set_user_not_approved(users, message, email = false)
    users.each do |user|
      user.set_approval_status_to_not_approved
      user.save
      Emailer.user_approval_removed(user).deliver_now if email
      note = <<~NOTE.squish
        User #{user.id},  #{user.email}, #{message}, so their approval status has been set to "not_approved".
      NOTE
