namespace :usasearch do
  namespace :recalls do
    ...
    task :load_cpsc_data, [:recalls_csv_filename] => [:environment] do |t, args|
      if args.recalls_csv_filename.blank?
        Rails.logger.error("usage: rake usasearch:recalls:load_cpsc_data[/path/to/recalls/csv]")
      else
    ...
    task :read_cpsc_feed, [:xml_url] => [:environment] do |t, args|
      if args.xml_url.blank?
        Rails.logger.error("usage: rake usasearch:recalls:read_cpsc_feed[XML Feed URL]")
      else
    ...
    task :read_nhtsa_feed, [:tab_delimited_url] => [:environment] do |t, args|
      if args.tab_delimited_url.blank?
        Rails.logger.error("usage: rake usasearch:recalls:read_nhtsa_feed[Tab-delimited Feed URL]")
      else
    ...
    task :load_nhtsa_data, [:data_file] => [:environment] do |t, args|
      if args.data_file.blank?
        Rails.logger.error("usage: rake usasearch:recalls:load_nhtsa_data[/path/to/recalls/file]")
      else
    ...
    task :load_cdc_data, [:rss_url, :food_type] => [:environment] do |t, args|
      if args.rss_url.blank? || args.food_type.blank?
        Rails.logger.error("usage: rake usasearch:recalls:load_cdc_data[RSS Feed URL, food type]")
      else
    ...
    task :load_sample_upc_data => :environment do
    ...
    task :load_upc_data, [:filename] => [:environment] do |t, args|
      if args.filename.blank?
        Rails.logger.error "Usage: rake usasearch:recalls:load_upc_data[/path/to/filename] Rails.env=your_env"
      else
