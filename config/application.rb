...
module UsasearchRails3
  class Application < Rails::Application
    ...
    config.autoload_paths += %W(#{config.root}/extras #{Rails.root}/lib #{Rails.root}/lib/importers #{Rails.root}/lib/middlewares #{Rails.root}/lib/parsers)
    ...
    unless File.basename($0) == "rake" && ARGV.include?("db:migrate")
      config.active_record.observers = :sayt_filter_observer, :misspelling_observer, :indexed_document_observer,
        :site_domain_observer, :affiliate_observer, :navigable_observer
    end
