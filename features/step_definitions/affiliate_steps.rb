Given /^the following Affiliates exist:$/ do |table|
  table.hashes.each do |hash|
    ...
    Affiliate.create(:name => hash["name"], :user=> user, :domains => hash["domains"], :header => hash["header"], :footer => hash["footer"], :staged_domains => hash["staged_domains"], :staged_header => hash["staged_header"], :staged_footer => hash["staged_footer"])
  end
end