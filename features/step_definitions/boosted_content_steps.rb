...
Given /^the following Boosted Content entries exist:$/ do |table|
  sites = table.hashes.collect do |hash|
    ...
    BoostedContent.create(:url => hash["url"],
                          :description => hash["description"],
                          :title => hash["title"],
                          :locale => hash["locale"].present? ? hash["locale"] : I18n.default_locale.to_s,
                          :auto_generated => hash['autogenerated'] == "true" ? true : false,
                          :keywords => hash["keywords"],
                          :status => hash['status'] || 'active',
                          :publish_start_on => publish_start_on,
...
When /^there are (\d+) Boosted Content entries exist:$/ do |count, table|
  boosted_contents = []
  table.hashes.each do |hash|
    ...
    count.to_i.times do |i|
      boosted_contents << BoostedContent.create!(:title => hash["title"] || "title #{i + 1}",
                                                 :description => hash["description"] || "description#{i + 1}",
                                                 :url => hash["url"] || "http://agency.gov/#{i + 1}/#{Time.now.to_f}.html",
                                                 :keywords => hash["keywords"],
                                                 :locale => hash['locale'].blank? ? I18n.locale.to_s : hash['locale'],
                                                 :auto_generated => hash['autogenerated'] == "true" ? true : false,
                                                 :status => hash['status'] || 'active',
