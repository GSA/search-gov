Given /^the following Boosted Content entries exist for the affiliate "([^\"]*)"$/ do |aff_name, table|
  ...
  sites = table.hashes.collect do |hash|
    BoostedContent.create!(:url => hash["url"], :description => hash["description"], :title => hash["title"], :affiliate => affiliate, :keywords => hash["keywords"], :locale => hash['locale'].blank? ? I18n.locale.to_s : hash['locale'], :auto_generated => hash['autogenerated'] == "true" ? true : false)
  end
