module AffiliateHelper
  ...
  def render_social_media_table_row(profile)
    ...
    preview_link = link_to('Recent Content', preview_social_media_affiliate_path(:profile_id => profile.id, :profile_type => profile.class.name)) unless profile.is_a?(FacebookProfile)
    delete_link = link_to('Delete', social_media_affiliate_path(:profile_id => profile.id, :profile_type => profile.class.name), :method => :delete)
  ...
  def render_title_for_social_media_preview(profile_type)
    return "Preview of recent YouTube videos" if profile_type == "YoutubeProfile"
    return "Preview of recent tweets" if profile_type == "TwitterProfile"
    return "Preview of recent Flickr photos" if profile_type == "FlickrProfile"
  end
