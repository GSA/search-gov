class SearchgovDomainIndexerJob < ActiveJob::Base
 ...
 # def perform(searchgov_domain:, delay:, conditions: fetch_required, start: Time.now.utc.to_s )
  def perform(searchgov_domain:, delay:, conditions:, start: Time.now.utc.to_s )
    ...
    if fetchable_urls.any?
      SearchgovDomainIndexerJob.set(wait: delay.seconds).
        perform_later(searchgov_domain: searchgov_domain, delay: delay, conditions: conditions, start: start)
    else
  ...
  #def fetch_required
   # 'last_crawled_at IS NULL OR lastmod > last_crawled_at'
  #end
