...
class ElasticsearchDeleteByQueryJob < ApplicationJob
  ...
  def perform
    ...
    begin
      ...
      if result[:completed]
        ...
        ES.client.tasks.cancel(task_id: task_id)
      end
  ...
  def with_start_retries(index, body, slices, requests_per_second, scroll_size)
    ...
    begin
      ...
      if attempt <= START_RETRY_ATTEMPTS
        ...
        Rails.logger.error { "Failed to start delete_by_query after #{attempt - 1} retries for index=#{index}: #{e.class}: #{e.message}" }
        raise
      end
    end
  end
  ...
  def wait_for_task_completion(task_id, max_wait_seconds)
    ...
    loop do
      ...
      begin
        resp = ES.client.tasks.get(task_id: task_id)
      rescue Elasticsearch::Transport::Transport::Errors::NotFound => e
