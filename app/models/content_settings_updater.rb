...
class ContentSettingsUpdater
  ...
  def update_affiliate_content_setings(affiliate)
    affiliate.header_logo_attachment = affiliate.mobile_logo if affiliate.mobile_logo_file_name.present?
    affiliate.header_logo.custom_metadata['alt_text'] = affiliate.logo_alt_text if affiliate.logo_alt_text.present?
    update_header_links(affiliate)
    update_footer_links(affiliate)
    affiliate.save
  end

  def migrate_paperclip_image_to_active_storage
  ...
  def log_completion(updated_list, failed_list)
    Rails.logger.info("[content_setings_updater_task] The following affiliates were updated successfully: #{updated_list}.") if updated_list.present?
    Rails.logger.error("[content_setings_updater_task] The following affiliates failed to update: #{failed_list}.") if failed_list.present?
  end
end
