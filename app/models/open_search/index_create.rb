...
class OpenSearch::IndexCreate
  def initialize(service_name:, index_name:, shards:, replicas:)
    @client = nil
    @service_name = service_name.upcase
    raise ArgumentError, \
      "service_name must be 'OPENSEARCH' or 'LEGACY_OPENSEARCH'" \
      unless %w[OPENSEARCH LEGACY_OPENSEARCH].include?(@service_name)
  ...
  def create_or_update_index(client)
    ...
    template_generator = OpenSearch::Template.new(index_pattern, @shards, @replicas)
    ...
    # elasticsearch-ruby (and OpenSearch) version we're using
