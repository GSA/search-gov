class BestBetImpressionsLogger
  class << self
    def log(affiliate_id, query, featured_collections, boosted_contents)
      log_best_bets(affiliate_id, query, 'BBG', featured_collections) if featured_collections and featured_collections.total > 0
      log_best_bets(affiliate_id, query, 'BOOS', boosted_contents) if boosted_contents and boosted_contents.results
    end
