...
class GoogleSearch < SearchEngine
  ...
  def process_results(response)
    ...
    processed = web_results.collect do |result|
      title = enable_highlighting ? convert_highlighting(result.html_title) : result.title
      content = enable_highlighting ? convert_highlighting(result.html_snippet) : result.snippet
      if title.present?
