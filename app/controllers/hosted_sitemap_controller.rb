class HostedSitemapController < ApplicationController
  ...
  def show
    @indexed_domain = IndexedDomain.find(params[:id]) rescue nil
    if @indexed_domain
      doc_count = @indexed_domain.indexed_documents.ok.size
      ...
      if doc_count > max_urls_per_sitemap and params[:page].blank?
        ...
        @indexed_documents = @indexed_domain.indexed_documents.ok.select(:url).limit(max_urls_per_sitemap).offset((page - 1) * max_urls_per_sitemap)
      end
    end
  end
