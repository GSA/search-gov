class Admin::AffiliatesController < Admin::AdminController
  ...
  active_scaffold :affiliate do |config|
    ...
    config.update.columns = [:display_name, :domains, :staged_domains, :search_results_page_title, :staged_search_results_page_title, :header, :staged_header, :footer, :staged_footer, :affiliate_template, :staged_affiliate_template, :is_sayt_enabled, :has_staged_content, :exclude_webtrends, :popular_urls, :locale]
    config.create.columns = [:display_name, :name, :domains, :header, :footer, :affiliate_template, :locale]
