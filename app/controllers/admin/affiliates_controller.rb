class Admin::AffiliatesController < Admin::AdminController
  ...
  active_scaffold :affiliate do |config|
    config.columns = [:display_name, :name, :domains, :affiliate_template, :boosted_contents, :is_sayt_enabled, :is_affiliate_suggestions_enabled, :created_at, :updated_at]
    config.list.sorting = { :display_name => :asc }
    config.update.columns = [:display_name, :domains, :staged_domains, :search_results_page_title, :staged_search_results_page_title, :header, :staged_header, :footer, :staged_footer, :affiliate_template, :staged_affiliate_template, :is_sayt_enabled, :is_affiliate_suggestions_enabled, :has_staged_content, :popular_urls]
