class Admin::AffiliatesController < Admin::AdminController
  ...
  active_scaffold :affiliate do |config|
    ...
    attribute_columns = config.columns.reject do |column|
      column.association or column.name =~ /(_created_at|_updated_at|agency_id|css_properties|content_type|file_name|_image|json|label|_logo|_mappings|scope_ids|size|status_id|uses_managed_header_footer)\z/
    end.map(&:name)
    ...
    enable_disable_column_regex = /^(is\_|dap_enabled|force_mobile_format|gets_blended_results|gets_commercial_results_on_blended_search|jobs_enabled|raw_log_access_enabled|search_consumer_search_enabled|search_consumer_admin_enabled|gets_i14y_results)/.freeze
