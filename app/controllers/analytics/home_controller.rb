class Analytics::HomeController < Analytics::AnalyticsController
  ...
  def queries
    ...
    @most_recent_day_popular_terms = DailyPopularQuery.find_all_by_day_and_time_frame_and_is_grouped(@day_being_shown, 1, false, :limit => @num_results_dqs, :order => 'times DESC')
    @trailing_week_popular_terms = DailyPopularQuery.find_all_by_day_and_time_frame_and_is_grouped(@day_being_shown, 7, false, :limit => @num_results_dqs, :order => 'times DESC')
    ...
    @trailing_month_popular_query_groups = DailyPopularQuery.find_all_by_day_and_time_frame_and_is_grouped(@day_being_shown, 30, true, :limit => @num_results_dqgs, :order => 'times DESC')
