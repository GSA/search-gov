...
To facilitate use of this container a makefile has been provided with the following 3 commands:
    ...
    pull-elastic - This pulls the desired container from dockerhub
    
    run-elastic - Runs the docker container with the correct ports
    
    test-elastic - Tests that the elastic search container is currently running
