...
describe SearchgovDomainIndexerJob do
  ...
  context 'when indexing only URLs meeting specific conditions' do
    ...
    let!(:failed_url) do
      SearchgovUrl.create!(url: 'https://agency.gov/404',
                           last_crawled_at: 1.week.ago,
    ...
    let(:args) do
      {
        searchgov_domain: searchgov_domain,
        delay: 10,
        conditions: { last_crawl_status: '404' },
        start: start
      }
    end
    ...
    it 'does not fetch the URLs that do not meet the conditions ' do
      expect{ perform }.not_to change{ ok_url.reload.last_crawled_at }
    end

    context 'when multiple URLs need to be fetched' do
      let!(:another_404) do
        SearchgovUrl.create!(url: 'https://agency.gov/404_b',
                             last_crawled_at: 1.week.ago,
                             last_crawl_status: '404')
      end
