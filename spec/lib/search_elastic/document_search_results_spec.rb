...
describe SearchElastic::DocumentSearchResults do
  ...
  describe '#results' do
    ...
    context 'when a PDF was indexed with a non-English language suffix' do
      let(:result) do
        { 'hits' => { 'total' => 1, 'hits' => [hits] },
          'aggregations' => {},
          'suggest' => [] }
      ...
      let(:hits) do
        { '_type' => '_doc',
          '_source' => { 'path' => 'https://example.gov/report.pdf',
                         'language' => 'de',
                         'mime_type' => 'application/pdf',
                         'title_de' => 'Annual Report' },
          'highlight' => { 'content_de' => ['Some highlighted PDF content'] } }
      end

      it 'deserializes title and highlight using the document language' do
        expect(results.first).to include(
          'language' => 'de',
          'title' => 'Annual Report',
          'content' => 'Some highlighted PDF content'
        )
      end
    end
