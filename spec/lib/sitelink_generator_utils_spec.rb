...
describe SitelinkGeneratorUtils do
  describe '.matching_generator_names' do
    it 'returns class name when the input url matches the url_prefix' do
      ...
      urls.each do |input_url|
        expect(SitelinkGeneratorUtils.matching_generator_names([input_url])).to eq(['SitelinkGenerator::SecEdgar'])
      end
    end
    ...
    it 'returns empty array when the input url does not match the url_prefix' do
      ...
      urls.each do |input_url|
        expect(SitelinkGeneratorUtils.matching_generator_names([input_url])).to be_empty
      end
    end
  end
  ...
  describe '.classes_by_names' do
    it 'returns valid classes' do
      ...
      expect(SitelinkGeneratorUtils.classes_by_names(class_names)).to eq([SitelinkGenerator::SecEdgar])
    end
  end
end
