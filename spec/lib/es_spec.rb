...
describe Es do
  ...
  context 'when working in Es::ELK submodule' do
    describe '.client_reader' do
      ...
      context 'when APP_OPENSEARCH_ENABLED is false' do
        before do
          allow(ENV).to receive(:[]).and_call_original
          allow(ENV).to receive(:[]).with('APP_OPENSEARCH_ENABLED').and_return('false')
      ...
      context 'when APP_OPENSEARCH_ENABLED is true' do
        # Since APP_OPENSEARCH_ENABLED=true is now the default in test environment,
    ...
    describe '.client_writers' do
      ...
      it 'uses the correct configuration values based on APP_OPENSEARCH_ENABLED flag' do
  ...
  describe 'when working in Es::CustomIndices submodule' do
    describe '.client_reader' do
      ...
      it 'always uses Elasticsearch configuration (ignores APP_OPENSEARCH_ENABLED)' do
        expect(host[:host]).to eq(URI(ENV.fetch('ES_HOSTS').split(',').first).host)
        expect(host[:user]).to eq(ENV.fetch('ES_USER'))
      end
