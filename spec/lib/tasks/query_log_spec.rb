...
describe "query_log rake tasks" do
  ...
  describe "usasearch:query_log" do
    ...
    describe "usasearch:search_api_query_log:transform_to_hive_queries_format" do
      before do
        @task_name = "usasearch:search_api_query_log:transform_to_hive_queries_format"
      end
      ...
      context "when passed a log file parameter" do
        it "should attempt to process the log file" do
          filename = "/path/to/file.log"
          SearchApiLogFile.should_receive(:transform_to_hive_queries_format).with(filename)
          @rake[@task_name].invoke(filename)
        end
      end
    end
