...
describe SitemapIndexer do
  ...
  describe '#index' do
    ...
    describe '#txt_sitemap?' do
      let(:txt_sitemap_url) { 'https://repodb.net/sitemap.txt' }
      let(:domain) { 'repodb.net' }
      let(:sitemap_indexer) { SitemapIndexer.new(sitemap_url: txt_sitemap_url, domain: domain) }

      context 'when #txt_sitemap? is true' do
        before do
          allow(sitemap_indexer).to receive(:https_url).and_return(txt_sitemap_url)
        end
      ...
      context 'process_entries when urls are extracted from txt format sitemap' do
