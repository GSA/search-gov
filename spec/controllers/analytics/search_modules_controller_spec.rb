...
describe Analytics::SearchModulesController do
  ...
  describe "#index" do
    ...
    context "when logged in as an analyst" do
      ...
      describe "locale" do
        context "when no locale is specified by user" do
          it "should show data from all locales" do
            get :index
            assigns[:locale_pick].should be_nil
          end
        end

        context "when locale param is blank" do
          it "should show data from all locales" do
            get :index, :locale_pick => ''
            assigns[:locale_pick].should be_nil
          end
        end

        context "when user selects a locale" do
          it "should use that locale for the locale being shown in the pick list" do
            get :index, :locale_pick => "es"
            assigns[:locale_pick].should == "es"
          end
        end
      end
