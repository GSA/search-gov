...
describe Admin::BulkUrlUploadController do
  ...
  include_context 'super admin logged in' do
    describe '#upload' do
      ...
      let(:fake_job_creator)  { spy(BulkUrlUploadJobCreator.new(uploaded_file, user)) }

      before { allow(BulkUrlUploadJobCreator).to receive(:new).and_return(fake_job_creator) }
      ...
      describe 'when the user has not selected a file' do
        ...
        it 'does not try to create the job' do
          expect(fake_job_creator).not_to have_received(:create_job!)
        end
        ...
        it 'does not show the user a success message' do
          expect(subject.request.flash[:success]).to be(nil)
        end

        it 'shows the user an error message' do
          expect(subject.request.flash[:error]).to eq('Please choose a file to upload.')
        end
      end
      ...
      describe 'when the user has selected a file' do
        ...
        it 'tries to create the job' do
          expect(fake_job_creator).to have_received(:create_job!)
        end
        ...
        it 'shows the user a success message' do
          expect(subject.request.flash[:success]).to eq("Successfully uploaded #{original_filename} for processing.")
        end

        it 'does not show the user an error message' do
          expect(subject.request.flash[:error]).to be(nil)
        end
      end
      ...
      describe 'when the BulkUrlUploadJobCreator throws an error' do
        ...
        before do
          allow(fake_job_creator).to receive(:create_job!).
        ...
        it 'shows the user the error message' do
          expect(subject.request.flash[:error]).to eq(error_message)
        end
      end
    end
  end
