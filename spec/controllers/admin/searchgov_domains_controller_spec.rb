  ...
  describe '#delete_domain' do
    context 'with correct confirmation text' do
      it 'enqueues the deletion job and redirects to index' do
        expect {
          post :delete_domain, params: { id: basic_domain.id, confirmation: 'DESTROY DOMAIN' }
    ...
    context 'with incorrect confirmation text' do
      it 'does not enqueue the deletion job and redirects to show' do
        expect {
          post :delete_domain, params: { id: basic_domain.id, confirmation: 'INCORRECT TEXT' }
        }.to_not have_enqueued_job(SearchgovDomainDestroyerJob)
