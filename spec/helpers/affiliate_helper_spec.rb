...
describe AffiliateHelper do
  ...
  describe "#site_wizard_header" do
    it "should add current_step class based on the current_step parameter" do
      ...
      content.should have_tag("img[alt=Step 1. Enter contact information]")

      content = helper.site_wizard_header :new_site_information
      content.should have_tag("img[alt=Step 2. Set up site]")

      content = helper.site_wizard_header :get_the_code
      content.should have_tag("img[alt=Step 3. Get the code]")
    end
  end
  ...
  describe "#render_choose_site_templates" do
    it "should display radio button, label and image for the template" do
      ...
      content.should have_tag("input[name='affiliate[staged_affiliate_template_id]'][value=#{affiliate_templates(:default).id}][checked=checked]")
      content.should have_tag("input[name='affiliate[staged_affiliate_template_id]'][value=#{affiliate_templates(:basic_gray).id}]")
    end
    ...
    it "should have basic gray affiliate template checked if staged_affiliate_template is set to basic gray" do
      ...
      content.should have_tag("input[name='affiliate[staged_affiliate_template_id]'][value=#{affiliate_templates(:default).id}]")
      content.should have_tag("input[name='affiliate[staged_affiliate_template_id]'][value=#{affiliate_templates(:basic_gray).id}][checked=checked]")
    end
  end
end
