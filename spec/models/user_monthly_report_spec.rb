...
describe UserMonthlyReport do
  ...
  let(:popular_queries) { [['query5', 53], ['query6', 43], ['query4', 42]] }
  let(:popular_clicks) { [['click5', 43], ['click6', 33], ['click4', 32]] }

  before do
    user.affiliates = [affiliates(:basic_affiliate)]
    allow(RtuMonthlyReport).to receive(:new).and_return(double(RtuMonthlyReport, total_queries: 100, total_clicks: 50),
  ...
  it 'should assign the affiliate stats' do
    stats = { affiliate: affiliates(:basic_affiliate), total_queries: 100, total_clicks: 50, last_month_total_queries: 80, last_year_total_queries: 1000, last_month_percent_change: 25.0, last_year_percent_change: -90.0, popular_queries: popular_queries, popular_clicks: popular_clicks }
