  ...
  describe '#fetch' do
    ...
    context 'when the url is redirected' do
      ...
      context 'on the client side' do
        let(:html) { "<header><meta http-equiv=\"refresh\" content=\"0; URL='/client_side.html'\"/></header>" }
        before { stub_request(:get, url).to_return(status: 200, body: html) }

        it 'creates a url' do
          expect(SearchgovUrl).to receive(:create).with(url: 'http://www.agency.gov/client_side')
          fetch
        end
      end
    end
