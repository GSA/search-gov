...
describe Template do
  describe 'schema' do
    it { is_expected.to have_db_column(:name).of_type(:string).with_options(null: false, limit: 50, index: true, unique: true) }
    it { is_expected.to have_db_column(:klass).of_type(:string).with_options(null: false, limit: 50, index: true, unique: true) }
