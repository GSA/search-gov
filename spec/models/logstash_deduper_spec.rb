...
describe LogstashDeduper, ".perform" do
  before do
    ...
    ES::client_reader.should_receive(:search).with(index: 'logstash-2015.08.26', type: "search", scroll: '5m',
                                                   size: LogstashDeduper::SCROLL_SIZE, search_type: :scan).and_return cursor
    ES::client_reader.should_receive(:scroll).with(scroll_id: 'c2NhbjsxOzE3NDcwMjc0OmVRMFNETWNtUnltN0xjd3dWNEFpVUE7MTt0b3RhbF9oaXRzOjE1MzIzMjc7', scroll: '5m').and_return scroll_1, scroll_2, scroll_3
  end

  it 'deletes the dupes' do
    ES::client_reader.should_receive(:bulk).with(body: [{ :delete => { :_index => "logstash-2015.08.26", :_type => "search", :_id => "abcde" } },
