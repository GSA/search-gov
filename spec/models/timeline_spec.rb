...
describe Timeline do
  ...
  describe "#new" do
    it "should generate a new Timeline object given a query" do
      Timeline.new("foo", @affiliate.name).should be_instance_of(Timeline)
    end
    ...
    context "when there are no searches for a term on a given day" do
      before do
        DailyQueryStat.create!(:day => Date.yesterday, :query => "most recent query", :times => 1, :affiliate => @affiliate.name)
        [2, 9, 11, 12, 15].each { |x| DailyQueryStat.create!(:day => Date.current.to_date - x.days, :query => "foo", :times => 1, :affiliate => @affiliate.name) }
      end
    ...
    context "when data does not extend back to 13 months ago" do
      before do
        DailyQueryStat.create!(:day => Date.yesterday, :query => "foo", :times => 1, :affiliate => @affiliate.name)
      end
    ...
    context "when data extends back to 13 months ago" do
      before do
        DailyQueryStat.create!(:day => Date.yesterday, :query => "foo", :times => 1, :affiliate => @affiliate.name)
        DailyQueryStat.create!(:day => start_date, :query => "foo", :times => 1, :affiliate => @affiliate.name)
      end
    ...
    context "when data extends back to more than 13 months ago" do
      before do
        DailyQueryStat.create!(:day => Date.yesterday, :query => "foo", :times => 1, :affiliate => @affiliate.name)
        DailyQueryStat.create!(:day => start_date, :query => "foo", :times => 1, :affiliate => @affiliate.name)
        DailyQueryStat.create!(:day => before_start_date, :query => "foo", :times => 2, :affiliate => @affiliate.name)
      end
    ...
    context "when data does not extend forward to the most recently populated date" do
      before do
        ...
        DailyQueryStat.create!(:day => Time.now.yesterday.yesterday.to_date, :query => "foo", :times => 1, :affiliate => @affiliate.name)
        DailyQueryStat.create!(:day => Time.now.yesterday.to_date, :query => "bar", :times => 1, :affiliate => @affiliate.name)
      end
    ...
    context "when Daily Query Stats exist for affiliates besides the default" do
      before do
        DailyQueryStat.delete_all
        %w{ usagov affiliate.gov }.each do |affiliate_name|
          DailyQueryStat.create!(:day => Date.yesterday, :query => 'foo', :times => 1, :affiliate => affiliate_name)
        end
      end
