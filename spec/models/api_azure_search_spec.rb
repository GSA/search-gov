describe ApiAzureSearch do
  ...
  context 'when initialized with a Bing V2 key' do
    ...
    it_behaves_like 'a commercial API search'

    describe '#new', skip: 'SRCHAR-2913: Azure is slated for removal' do
      before do
    ...
    describe '#run', skip: 'SRCHAR-2913: Azure is slated for removal' do
      ...
      context 'when enable_highlighting is enabled' do
        ...
        it 'does not highlight display_url' do
          expect(search.results.first.display_url).not_to match(/\ue000.+\ue001/)
        end
      ...
      context 'when response _next is not present' do
        ...
        before do
          ...
          stub_request(:get, /#{AzureEngine::API_HOST}.*azure no next/o).
    ...
    describe '#as_json', skip: 'SRCHAR-2913: Azure is slated for removal' do
      subject(:search) do
      ...
      it 'does not hightlight display_url' do
        result = Hashie::Mash.new(search.as_json[:web][:results].first)
        expect(result.display_url).not_to match(/\ue000.+\ue001/)
      end

      it_behaves_like 'an API search as_json'
      it_behaves_like 'a commercial API search as_json'
    end
  end
  ...
  context 'when initialized with a Bing V5 key' do
    ...
    describe '#new', skip: 'SRCHAR-2913: Azure/BingV5 is slated for removal' do
      before do
        affiliate.site_domains.create!(domain: 'whitehouse.gov')
        affiliate.excluded_domains.create!(domain: 'kids.usa.gov')
      end
    ...
    describe '#run', skip: 'SRCHAR-2913: Azure/BingV5 is slated for removal' do
      context 'when offset is 0' do
        it 'initializes GovboxSet' do
          highlighting_options = {
            highlighting: true,
      ...
      it 'when response _next is not present', skip: 'SRCHAR-2913: Azure/BingV5 is slated for removal' do
        subject(:search) do
          described_class.new(search_params.merge({ query: 'azure no next' }))
        end
      ...
      it 'when the site locale is es', skip: 'SRCHAR-2913: Azure/BingV5 is slated for removal' do
        let(:search) do
          described_class.new(search_params.merge({ query: 'educación' }))
        end
      ...
      it 'when Azure response contains empty results', skip: 'SRCHAR-2913: Azure/BingV5 is slated for removal' do
        subject(:search) do
          described_class.new(search_params.merge({ query: 'mango smoothie' }))
        end
    ...
    describe '#as_json' do
      ...
      before do
        # SRCH-3125: Yes, this is being stubbed with a BingV7 response, not BingV5, but it is what we already have in
        # the code base. Since Azure/BingV5 are slated for removal, this was the lightest lift to merely ensure
        # adequate code coverage during the removal of Google.
        results = Rails.root.join('spec/fixtures/json/bing_v7/web_search/gov.json').read
        stub_request(:get, /#{BingV5WebEngine::API_HOST}.*gov/o).
      ...
      it 'returns results' do
        ...
        expect(result.title).to match(/Making government services easier to find | USAGov/)
        expect(result.snippet).to start_with('Find government benefits, services, agencies, and information')
        expect(result.url).to match(URI::DEFAULT_PARSER.make_regexp)
      end

      it 'highlights title and description', skip: 'SRCHAR-2913: Azure/BingV5 is slated for removal' do
        result = Hashie::Mash.new(search.as_json[:web][:results].first)
        expect(result.title).to match(/\ue000.+\ue001/)
        expect(result.snippet).to match(/\ue000.+\ue001/)
        expect(result.url).to match(URI::DEFAULT_PARSER.make_regexp)
      ...
      it 'does not hightlight display_url', skip: 'SRCHAR-2913: Azure/BingV5 is slated for removal' do
        result = Hashie::Mash.new(search.as_json[:web][:results].first)
        expect(result.display_url).not_to match(/\ue000.+\ue001/)
      end
