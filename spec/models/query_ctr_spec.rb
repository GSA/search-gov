...
describe QueryCtr do
  ...
  describe "#query_ctrs" do
    context "when stats are available for the range" do
      ...
      before do
        expect(ES::client_reader).to receive(:search).with(hash_including(type: %w(search click))).and_return(historical_query_json_response, query_json_response)
      end
    ...
    context "when no stats are available for the daterange" do
      before do
        expect(ES::client_reader).to receive(:search).with(hash_including(type: %w(search click))).twice.and_return nil
      end
